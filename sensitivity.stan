// Sensitivy analysis: modeling persistent IgM 
// Tokyo and Hyogo
...
transformed parameters {
  ...
  for (k in 1:K)
  for (t in 1:(T-1))
  inf_pre[t,k] = (1-prev[k])*Preg[t+1, k]*(1-exp(-foi[k]));
  
  real inf[T, K]; // infected at the first month of pregnancy
  for (k in 1:K)
  for (t in 1:T)
  inf[t, k] = (1-prev[k])*Preg[t, k]*(1-exp(-foi[k]));
  
  real inf_post[T, K]; // infected at the second month of pregnancy
  for (k in 1:K)
  for (t in 2:T)
  inf_post[t, k] = (1-prev[k])*Preg[t-1, k]*exp(-foi[k])*(1-exp(-foi[k]));
  
  real prev_preg[T, K]; // incident pregnancy ever-infected who will be given spiramycin
  for (k in 1:K)
  for (t in 1:T)
  prev_preg[t, k] = prev[k]*Preg[t,k]*0.1*0.1;// rate of not doing IgG avidity test is 10%; persistent IgM antibodies is 10% 
  ...
  for (k in 1:K)
  for (t in 14:T)
  c[t, k] = 0.7*prop[k]*(inf_pre[t-2, k] + inf[t-2, k] + inf_post[t-1, k] + prev_preg[t-2, k]);// 70% consent to recieve treatment
  ...
  for (k in 1:K)
  for (t in 10:T)
  dose_m[t, k] = 180*(c[t, k]+c[t-1, k]+c[t-2, k]+c[t-3, k]+c[t-4, k]+c[t-5, k]+c[t-6, k]+c[t-7, k]+c[t-8, k]);
...
generated quantities{
  ...
  for (k in 1:K){
    for (t in 3:T)
    inf_2m[t, k] = (1-prev[k])*Preg[t-2, k]*exp(-foi[k])^2*(1-exp(-foi[k]));
    for (t in 4:T)
    inf_3m[t, k] = (1-prev[k])*Preg[t-3, k]*exp(-foi[k])^3*(1-exp(-foi[k]));
    for (t in 5:T)
    inf_4m[t, k] = (1-prev[k])*Preg[t-4, k]*exp(-foi[k])^4*(1-exp(-foi[k]));
    for (t in 6:T)
    inf_5m[t, k] = (1-prev[k])*Preg[t-5, k]*exp(-foi[k])^5*(1-exp(-foi[k]));
    for (t in 7:T)
    inf_6m[t, k] = (1-prev[k])*Preg[t-6, k]*exp(-foi[k])^6*(1-exp(-foi[k]));
    for (t in 8:T)
    inf_7m[t, k] = (1-prev[k])*Preg[t-7, k]*exp(-foi[k])^7*(1-exp(-foi[k]));
    for (t in 9:T)
    inf_8m[t, k] = (1-prev[k])*Preg[t-8, k]*exp(-foi[k])^8*(1-exp(-foi[k]));
    for (t in 10:T)
    inf_9m[t, k] = (1-prev[k])*Preg[t-9, k]*exp(-foi[k])^9*(1-exp(-foi[k]));
    for (t in 10:(T-1))
